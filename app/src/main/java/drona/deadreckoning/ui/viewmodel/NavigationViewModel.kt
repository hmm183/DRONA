...
import drona.deadreckoning.domain.state.*
...
class NavigationViewModel(
    ...
    fun selectAlternativeRoute(alternativeId: String) {
        ...
        val updatedAlternatives = mutableListOf<drona.deadreckoning.domain.model.RouteAlternative>()
        updatedAlternatives.add(
            drona.deadreckoning.domain.model.RouteAlternative(
    ...
    fun recalculateRoute(currentPosition: GeoPoint, destinationPoint: GeoPoint, destinationName: String) {
        ...
        if (!drona.deadreckoning.util.RouteRerouteGating.isGnssTrustworthyForReroute(gnssState.value, navigationState.value, hasFreshGnss())) {
