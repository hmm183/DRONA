package drona.deadreckoning.fusion

import drona.deadreckoning.fusion.math.MatrixMath
...
class VehicleFusionImmUkf(
    ...
    override fun updateMapConstraint(
        ...
        // Along-track correction: pull position toward matched point
        val alongGain = 0.08 * (confidence / 100.0).coerceIn(0.0, 1.0)
        for (m in 0 until NUM_MODES) {
            modeStates[m][IDX_PE] += alongGain * innovE
            modeStates[m][IDX_PN] += alongGain * innovN
        }

        // Cross-track correction (only when bearing is known)
        ...
        combineOutput()
        return MapConstraintResult(true, state()!!, innovE * innovE + innovN * innovN)
    }
