...
import drona.deadreckoning.core.gnss.GnssFix
...
data class SyntheticImuStep(
    ...
    val accelForwardMps2: Double = 0.0,
    val accelLateralMps2: Double = 0.0
)
...
class SyntheticSensorGenerator(
    ...
    fun generateImuStep(gt: GroundTruthState, dt: Double): SyntheticImuStep {
        ...
        // True physical vehicle accelerations (tangential + centripetal)
        val trueYawRateRadPerSec = Math.toRadians(gt.yawRateDps)
        val centripetalAccel = gt.speedMps * trueYawRateRadPerSec
        val accelNoiseFwd = random.nextGaussian() * 0.05
        val accelNoiseLat = random.nextGaussian() * 0.05
        val aFwd = gt.accelMps2 + accelNoiseFwd
        val aLat = centripetalAccel + accelNoiseLat
        ...
        return SyntheticImuStep(
            ...
            accelForwardMps2 = aFwd,
            accelLateralMps2 = aLat
        )
    }
    ...
    fun generateAiSpeed(gt: GroundTruthState): SyntheticAiSpeed {
        ...
        val motionClass = when (gt.motionMode) {
            drona.deadreckoning.fusion.VehicleFusionImmUkf.MotionMode.CONSTANT_TURN_RATE -> MotionClass.TURNING
            drona.deadreckoning.fusion.VehicleFusionImmUkf.MotionMode.CONSTANT_ACCELERATION -> MotionClass.STRAIGHT
            drona.deadreckoning.fusion.VehicleFusionImmUkf.MotionMode.CONSTANT_VELOCITY -> MotionClass.STRAIGHT
        }
    ...
    fun generateGnss(gt: GroundTruthState, currentStepSec: Double): SyntheticGnssObservation {
        val inBlackout = config.isInBlackout(gt.progressFraction)
